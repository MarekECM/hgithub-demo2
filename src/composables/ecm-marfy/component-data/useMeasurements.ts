...
async function addMeasurementFormSubmit(
  data: AddMeasurementModel,
  ...
  try {
    const res = await axios.post(
      `${import.meta.env.VITE_API_URL}Device/AddOrEditMeasurement`,
      normalizedData,
      {
        headers: {
    ...
    toast.add({
      severity: 'success',
      summary: 'Povedlo se',
      detail: `Veličinu se povedlo vytvořit'`,
      life: 3000
    })
  } catch (e: any) {
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: `Veličina nebyla vytvořena : ${e.message}`,
      life: 3000
    })
  }
}
