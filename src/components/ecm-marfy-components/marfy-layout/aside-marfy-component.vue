...
import { useTreeNavigation } from '@/composables/ecm-marfy/component-aside/useTreeNavigation'
import { useResizeSidebar } from '@/composables/ecm-marfy/component-aside/useResizeSidebar'
...
async function handleNodeClick(node: any) {
  ...
  const dashboards = await getDashboards(
    node.id ?? 0,
    store.selectedOrgId ?? 0,
    node.deviceTypeID == null
  )
...
onMounted(() => {
  ...
  if (resize && sidebar && treeItems.length > 0) {
    ...
    console.error(
      'Elementy .ecm-aside__resize, .ecm-aside nebo .ecm-aside__nav-tree-container nebyly nalezeny.'
    )
  }
})
</script>
...
<template>
  <aside class="ecm-aside" :class="{ 'is-visible': uiStore.sidebar }">
    <div class="ecm-aside__header">
      ...
      <div class="ecm-aside__logo">
        <RouterLink :to="{ name: 'homeView' }">
          <img
            class="ecm-aside__logo-image"
            src="/src/components/img/logo_ecm_dark.png"
            alt="logo"
          />
        </RouterLink>
      </div>
    ...
    <div
      class="ecm-aside__search ecm-aside__search--hidden"
      :class="{ 'ecm-aside__search--active': mainSelect.isActive }"
    >
    ...
    <div class="ecm-aside__footer">
      <span class="ecm-aside__footer-section ecm-aside__footer-section--top"
        >© 2024 - ECM System Solutions s.r.o.</span
      >
      <span class="ecm-aside__footer-section ecm-aside__footer-section--bottom"
        >ECM marfy Verze 0</span
      >
    </div>
